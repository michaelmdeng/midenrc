...
syn keyword	scopeKeywords		REFERENCE RESOURCE MODULE VIEW FUNCTION IMPORT PARAMS LOOP
syn keyword	scopeKeywords		DEFINE BEGIN FUNC RETURN ROWSET END CREATE VIEW SCHEMA
syn keyword	scopeKeywords		PRESORT ASC DESC WHERE SAMPLE PERCENT HAVING INNER JOIN OUTER LEFT RIGHT FULL SEMIJOIN
syn keyword	scopeKeywords		UNION EXCEPT ALL DISJOINT INTERSECT DISTINCT REGEX ON TO FROM GROUP ORDER BY AS LOCAL CROSS APPLY
...
syn keyword	scopeKeywords		SPARSE STREAMSET PATTERN
...
syn keyword	scopeSpecialColumns	__date __hour __datetime __serialnum
...
syn region	scopeInlinedCsharp	start="^\s*#CS" end="#ENDCS"me=s-1 keepend contains=@scopeCsharp,scopeCSStart,scopeCSEnd

" Comments
syn region	scopeCommentL		start="//" skip="\\$" end="$" keepend
syn region	scopeComment		matchgroup=scopeCommentStart start="/\*" end="\*/" extend

" Constants and variables (mostly taken from cs.vim, fixed verbatim strings
" with '@"' at the end)
...
syn region	scopeVerbatimString	start=+@"+hs=s+1 end=+"+ skip=+""+ contains=scopeInputVariable,@Spell
syn region	scopeString		start=+"+  end=+"+ end=+$+ contains=scopeSpecialChar,scopeSpecialError,scopeUnicodeNumber,scopeInputVariable,@Spell
...
hi def link scopeSpecialColumns		Type
